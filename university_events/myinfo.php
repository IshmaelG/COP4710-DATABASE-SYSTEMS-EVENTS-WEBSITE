	...
	<div class="container text-light">
		...
		<div class="row">
			...
			<div class="col-md pt-3">
				<div class="pb-5"><h3>My Info</h3></div>
				<?php
					$sql="SELECT name,uname,email,uid FROM users WHERE uname='".$_SESSION['uname']."'";
					$result = mysqli_query($connection,$sql);
					$info = mysqli_fetch_all($result, MYSQLI_ASSOC);

					foreach($info as $info){?>
						<p>Name: <?php echo htmlspecialchars($info['name']);?></p>
						<p>Username: <?php echo htmlspecialchars($info['uname']);?></p>
						<p>E-mail: <?php echo htmlspecialchars($info['email']);?></p>
						<?php
							$sql="SELECT name FROM university WHERE uniID='".$uid."'";
							$result = mysqli_query($connection,$sql);
							$infouid = mysqli_fetch_all($result, MYSQLI_ASSOC);

							foreach($infouid as $infouid){?>
								<p><?php echo htmlspecialchars($infouid['name']);
								echo ' (';
								echo htmlspecialchars($info['uid']);
								echo ')'?></p>
							<?php }?>
					<?php }?>
			</div>
